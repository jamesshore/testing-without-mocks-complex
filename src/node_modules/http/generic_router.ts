...
export type Routes<C> = Record<string, Controller<C>>;

export interface Controller<C> {
	getAsync?: RequestHandler<C>,
	postAsync?: RequestHandler<C>,
}

export type RequestHandler<C> = (request: HttpServerRequest, context?: C) => HttpServerResponse | Promise<HttpServerResponse>;
export type ErrorHandler = (status: number, description: string, request: HttpServerRequest) => HttpServerResponse | Promise<HttpServerResponse>;
...
export class GenericRouter<C> {

	private readonly _listener: OutputListener<GenericRouterRequests<C>>;
	 ...
	 */
	static create<C>(errorFnAsync: ErrorHandler, routes: Routes<C>): GenericRouter<C> {
		return new GenericRouter(errorFnAsync, routes);
	}

	/** Only for use by tests. (Use a factory method instead.) */
	constructor(private readonly _errorFnAsync: ErrorHandler, private readonly _routes: Routes<C>) {
	 ...
	 */
	async routeAsync(request: HttpServerRequest, log: Log, context?: C) {
	 ...
	 */
	trackRequests(): OutputTracker<GenericRouterRequests<C>> {
		return this._listener.trackOutput();
	}

	#recordRequest(method: string, path: string, log: Log, context?: C) {
		this._listener.emit({ method, path, context });
		log.info({ message: "request", method, path });
	}

	async #callRouteHandlerAsync(method: string, path: string, request: HttpServerRequest, context?: C) {
		const route = this._routes[path];
		if (route === undefined) return await this._errorFnAsync(404, "not found", request);

		const functionName = `${method}Async` as keyof Controller<C>;
