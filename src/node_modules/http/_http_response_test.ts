...
import { HttpHeaders, HttpServerResponse, HttpServerResponseOptions } from "http/http_server_response.js";
...
describe("HttpServerResponse", () => {
	...
	describe("generic factory", () => {
		...
		it("doesn't change response headers when original header object changes", () => {
			const headers: HttpHeaders = { "my-header": "my-value" };
			const response = HttpServerResponse.create({
				status: 999,
				body: "irrelevant body",
				headers,
...
function assertResponseEquals(actual: HttpServerResponse, expectedOptions: HttpServerResponseOptions) {
