...
module.exports = class HttpServer {
	...
	get log() {
		return this.isStarted ? this._log : null;
	}

	get config() {
		return this.isStarted ? this._config : null;
	}


	async startAsync(port, log, passThroughConfig, requestHandlerFnAsync) {
		ensure.signature(arguments, [ Number, Log, [ Object, null ], Function ]);
		if (this.isStarted) throw new Error("Can't start server because it's already running");

		this._port = port;
		this._log = log;
		this._config = passThroughConfig;
	...
	async #handleRequestAsync(httpRequest) {
		try {
			logRequest(this._log, httpRequest);

			const response = await this._requestHandlerFnAsync(httpRequest, this._config);
...
function logRequest(log, httpRequest) {
	log.info({
		message: "request",
		method: httpRequest.method,
		url: httpRequest.urlPathname,
	});
}
