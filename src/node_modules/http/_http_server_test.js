...
describe("HTTP Server", () => {
	...
	describe("starting and stopping", () => {
		...
		it("provides router", async () => {
			const { server } = createServer();
			assert.equal(server.router, null, "before server started");
			await startAndStopAsync({ server }, ({ router: expectedRouter }) => {
				assert.equal(server.router, expectedRouter, "after server started");
			});
			assert.equal(server.router, null, "after server stopped");
		});
	...
	describe("requests and responses", () => {
		...
		it("runs a function when a request is received and serves the result", async () => {
			...
			function routeAsync() { return HttpResponse.create(expectedResponse); }
	...
	describe("request simulation", () => {
		...
		it("simulates requests", async () => {
			...
			const { request: requestSent, response: responseReceived } = await simulateRequestAsync({ routeAsync });
...
async function getAsync({
	...
	return await startAndStopAsync({ routeAsync, config }, async ({ logOutput }) => {
		logOutput.clear();
		return {
...
async function startAndStopAsync(options, fnAsync = () => {}) {
	const { server, logOutput, router } = await startAsync(options);
	try {
		return await fnAsync({ logOutput, server, router });
	}
	finally {
		await stopAsync({ server });
	}
...
async function startAsync({
	...
	const router = { routeAsync };
	const logOutput = log.trackOutput();
	await server.startAsync(PORT, log, router);
	return { server, logOutput, router };
}
