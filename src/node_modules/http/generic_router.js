...
module.exports = class GenericRouter {
	...
	async routeAsync(request, log, config) {
		...
		recordRequest(this);
		return await routeRequestAsync(this);

		function recordRequest(self) {
			self._emitter.emit(REQUEST_EVENT, { method, path, config });
			log.info({ message: "request", method, path });
		}

		async function routeRequestAsync(self) {
			const route = self._routes[path];
			if (route === undefined) return await self._errorFnAsync(404, "not found", request);

			const functionName = `${method}Async`;
			const methodFnAsync = route[functionName];
			if (methodFnAsync === undefined) return await self._errorFnAsync(405, "method not allowed", request);
