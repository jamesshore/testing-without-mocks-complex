...
describe("Log", () => {
	...
	it("checks for equality", () => {
		const { log: log1a } = createLog();
		const { log: log1b } = createLog();
		const log2a = log1a.bind({ "my-default": "my-value" });
		const log2b = log1a.bind({ "my-default": "my-value" });

		assert.equal(log1a.equals(log1b), true, "log1a equals log1b");
		assert.equal(log2a.equals(log2b), true, "log2a equals log2b");

		assert.equal(log1a.equals(log2a), false, "log1a doesn't equal log2a");
	});
...
function createLog() {
	...
	const output = log.trackOutput();
	return { log, stdout: stdout.data, output: output.data };
}
