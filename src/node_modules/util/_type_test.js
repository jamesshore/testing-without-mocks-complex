...
describe("Type", function() {
	...
	describe("describer", function() {
		...
		it("describes 'structs' when extra keys are allowed", function() {
			...
			assert.equal(describe(Object.create(null), options), "object", "no prototype");
			assert.equal(describe({}, options), "object", "empty object");
			assert.equal(describe({ a: Number }, options), "object containing at least { a: <number> }", "one parameter");
			assert.equal(describe({ a: Number, b: String }, options), "object containing at least { a: <number>, b: <string> }");
		});
