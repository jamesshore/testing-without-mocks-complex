...
describe("Type", () => {
	...
	describe("checker", () => {
		...
		it("checks undefined and null types (primarily for allowing nullable objects, etc.)", () => {
		...
		it("checks NaN (just in case you ever want it)", () => {
		...
		it("has a special error message when comparing structs to instances, if they have extra parms", () => {
			// This will typically happen when forgetting to enclose a config parameter in an object. So we have a special
			// error message to make the situation more obvious.

			class MyClass {
				private readonly _randomVariable?: boolean;
				constructor() {
					this._randomVariable = true;
				}
			}
			const myObject = new MyClass();

			assert.equal(
				check(myObject, { myObject: [ undefined, MyClass ]}),
				"argument should probably be an anonymous object, but it was a MyClass instance " +
				"(argument had unexpected parameter: _randomVariable). Did you forget to put it in curly braces?",
				"failure"
			);
			assert.equal(
				check(myObject, { _randomVariable: Boolean }),
				null,
				"success",
			);
		});

		it("provides correct error message when receiving a struct and expecting a primitive", () => {
			assert.equal(
		...
		it("provides class names when using class instances", () => {
		...
		it("allows argument name to be customized in error message", () => {
	...
	describe("describer", () => {
		...
		it("describes 'structs' when extra keys are allowed", () => {
		...
		it("optionally uses articles", () => {
			class MyClass {}
			const forceAnonymity = {} as any;
			const AnonClass = forceAnonymity.whatever = class {};
